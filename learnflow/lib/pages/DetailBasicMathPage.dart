// lib/pages/DetailBasicMathPage.dart  [UPDATED — ซ่อน Retake ถ้ายังไม่เคยทำ]
...
class DetailBasicMathPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // best_score มีค่า (ไม่ใช่ null และไม่ใช่ 0) = เคยทำมาแล้ว → แสดง Retake
    final bestScore = quiz['best_score'];
    final hasDone   = bestScore != null && bestScore != 0;
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildQuizDetailsSection(subject, level, totalQ, bestScore),
                    const SizedBox(height: 24),
                    _buildActionButtons(context, quizId, hasDone),
                    const SizedBox(height: 20),
                  ],
                ),
              ),
    ...
    return Center(
      child: Container(
        ...
        child: assetPath != null
            ...
            : const Icon(Icons.quiz_outlined, size: 56, color: primaryGreen),
      ),
    );
  }

  Widget _buildQuizDetailsSection(String subject, String level, dynamic totalQ, dynamic bestScore) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        if (bestScore != null && bestScore != 0) ...[
          _buildDetailRow('BEST SCORE :', '$bestScore%'),
          const SizedBox(height: 8),
        ],
  ...
  Widget _buildActionButtons(BuildContext context, int quizId, bool hasDone) {
    return Column(children: [
      ...
      // แสดง RETAKE เฉพาะเมื่อเคยทำ quiz ไปแล้ว
      if (hasDone) ...[
        const SizedBox(height: 12),
